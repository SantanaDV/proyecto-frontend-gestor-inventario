...
export default function Inventario() {
  ...
  const handleModalOpen = () => {
    const newId = parsedData.length 
      ? Math.max(...parsedData.map(p => Number(p.id) || 0)) + 1
      : 1;
    ...
    {productosActivos.length === 0 ? <p>No hay productos activos.</p> : productosActivos.map(producto => (
      <div key={producto.id_producto} className="border border-gray-300 rounded-lg shadow-md text-center bg-green-50 p-4 mb-4 flex flex-col items-center">
       <div className="flex w-full justify-between mb-4">
          <h3 className="text-lg font-semibold text-blue-600">{producto.nombre}</h3>
          <div className="flex gap-2">
            <button><img className="w-6 h-6" src="editar.png" alt="Editar" /></button>
            <button><img className="w-6 h-6" src="eliminar.png" alt="Eliminar" /></button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-600 bg-opacity-50">
          <div className="bg-white p-6 rounded-lg w-1/3">
            ...
            <div className="flex justify-between">
                ...
                <button type="submit" className="bg-red-500 text-white px-4 py-2 rounded">Guardar</button>
            </div>
            </form>
          </div>
        </div>
